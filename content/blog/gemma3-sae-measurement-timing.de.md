---
title: "SparseAutoencoders können Generationszeit-Verhalten nicht messen. Das ist kein Bug."
date: 2026-04-07
tags: ["ai", "interpretability", "sparse-autoencoders"]
description: "Warum sycophancySAE Features haben Cohen's d=9.9, aber Halluzinationserkennung scheitert. Die Antwort stellte sich als tiefer heraus als die Messzeit."
...
---
**Dein Messfenster bestimmt, welche Verhaltensweisen du sehen kannst. Sycophancy tritt während des Encodings auf. Halluzination tritt während der Generierung auf. Wenn du die falsche Timing nutzt, kollabiert dein Cohen's d.**

Ich habe letzte Woche zwei Stunden damit verbracht, auf einem Gemma3 sparse autoencoder (SAE) Feature‑Diagramm zu starren und zu fragen, warum die Sycophancy‑Erkennung perfekt funktionierte (Cohen's d etwa 9,9), während die Halluzination‑Detektion flach blieb (d < 1,0). Gleiches Modell. Gleiches SAE. Gleiches Verfahren. Die Fehlerbalken überschnitten sich nicht. Das wäre unmöglich, wenn SAEs tatsächlich "Verhaltensmerkmale" finden, so wie die Interpretierbarkeits‑Community behauptet.

Dann fiel es mir ein: die Timing war falsch.
