---
title: "Sparse Autoencoders können das Generations‑Zeit‑Verhalten nicht messen. Das ist kein Bug."
date: 2026-04-07
...
---
**Your measurement windowdetermines what behaviors you can see. Sycophancy manifests during encoding. Hallucination manifests during generation. Use the wrong timing and your Cohen's d collapses.**

"Ich habe letzte Woche zwei Stunden damit verbracht, an einem Gemma3 sparse Autoencoder (SAE) Feature‑Diagramm zu starren und zu fragen, warum die Sycophancy‑Erkennung einwandfrei funktionierte (Cohen's d etwa 9,9), während die Halluzinations‑Erkennung fast keine Aktivität zeigte (d < 1,0). Das gleiche Modell. Das gleiche SAE. Derselbe Vorgehen. Die Konfidenzintervalle überschneiden sich nicht. Das sollte unmöglich sein, wenn SAEs tatsächlich \"behavioral features\" auf die Weise finden, wie die Interpretierbarkeit‑Community behauptet."
...
*The cover image for this post was generated by AI.*
## Was ich weggelassen habe**Warum SAE‑Forschung standardmäßig Zeitmessungen zur Kodierung verwendet.** Kodierungszeit‑Aktivierungen sind zustandslos und deterministisch. Generierungszeit‑Aktivierungen hängen von der gesamten Sequenzgeschichte ab und sind über Temperatur und Sampling stochastisch. Die Mathematik ist bei der Kodierungszeit übersichtlicher. Aber saubere Mathematik für das falsche Problem ergibt saubere, aber nutzlose Ergebnisse.

**Verhaltensschaltkreise jenseits von SAEs.** Sparse Autoencoder ist nur eine Perspektive. Kausale Intervention (Ablation) ist eine weitere. Analyse von Aufmerksamkeitsmustern ist eine dritte. Jede Basis enthüllt unterschiedliche Verhaltensweisen. Ein vollständiges Bild erfordert mehrere Messmethoden über mehrere Phasen hinweg. Dieser Beitrag beschäftigt sich nur mit SAE + Kodierungszeit.

**Warum Halluzination schwer und Schmeichelei leicht ist.** Dies geht mit der größeren Frage einher, ob Modellausrichtung über Verhaltenslenkung erreichbar ist oder architecturale Änderungen erfordert. Wenn alle besorgniserregenden Verhaltensweisen in Generierungszeitphasen zusammenfallen und für die Kodierungszeitmessung unsichtbar bleiben, könnte die gesamte Agenda der Interpretierbarkeit der Encodierungsschicht die eigentlichen Versagensmodi übersehen. Das ist einen eigenen Beitrag wert.
...
*Christian Pojoni baut KI‑Tools und Interpretierbarkeitsinfrastruktur. Mehr bei vasudev.xyz.*
