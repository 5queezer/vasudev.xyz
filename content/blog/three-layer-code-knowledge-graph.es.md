---
title: "Su Grafo de Conocimiento de Código Necesita Cuatro Capas, No Una"
date: 2026-05-01
tags: ["agents", "ai", "architecture"]
agentQuestions:
  - "¿Por qué un grafo de conocimiento de código necesita cuatro capas?"
  - "¿Qué agrega la capa cuatro para los agentes?"
  - "¿Cómo debe una LLM navegar un grafo de código?"
...
**Un grafo de conocimiento de código para un agente LLM debería tener cuatro capas. La capa dos hace la navegación. La capa cuatro hace el trabajo.**
...
Tres corrientes convergen en esta forma y, en su mayor parte, no se hablan entre sí.

La primera es el análisis de programas. El **code property graph** ([enlace](https://en.wikipedia.org/wiki/Code_property_graph)), introducido por Yamaguchi et al. en su artículo de IEEE S&P 2014 “Modeling and Discovering Vulnerabilities with Code Property Graphs” (que ganó el IEEE Test‑of‑Time Award en 2024), ya fusiona tres representaciones clásicas en una sola estructura: el árbol de sintaxis abstracta, el grafo de flujo de control y el grafo de dependencia del programa. El caso de uso original era el descubrimiento de vulnerabilidades, pero la lección se generaliza. Una única representación no puede responder todas las preguntas que tienes sobre el código, por lo que compones representaciones y dejas que la consulta elija el segmento adecuado. Eso es la capa uno bien hecha, y se ha hecho bien durante más de una década.

La segunda es la recuperación basada en grafos para LLMs. El artículo de Microsoft GraphRAG, **“From Local to Global: A Graph RAG Approach to Query‑Focused Summarization”** ([enlace](https://arxiv.org/abs/2404.16130)) de Edge et al. (2024), es explícito sobre el valor de una capa intermedia de comunidad. Construyen un grafo de entidades, lo particionan con el **algoritmo Leiden** ([enlace](https://en.wikipedia.org/wiki/Leiden_algorithm)) y generan resúmenes por comunidad. La **documentación de GraphRAG** ([enlace](https://microsoft.github.io/graphrag/)) es directa sobre lo que esas comunidades te aportan: cada nivel de la jerarquía resultante “representa un nivel diferente de abstracción y resumen”. Esa es la estructuración por capas en sus propias palabras, aplicada a documentos en lugar de código. Las consultas golpean primero los resúmenes de comunidad y sólo descienden a las entidades cuando es necesario. *La Capa de Memoria* describe el mismo patrón en el Capítulo 5 y trata a HybridRAG (una mezcla ajustable `α · vector_score + (1 - α) · graph_score`) como el nuevo predeterminado. Ambos confirman que la capa de comunidad es real y estructuralmente necesaria.

El trabajo reciente centrado en código está convergiendo en la misma forma. **“Code Graph Model (CGM): A Graph‑Integrated Large Language Model for Repository‑Level Software Engineering Tasks”** ([enlace](https://arxiv.org/abs/2505.16901)) de Tao et al. integra la estructura del grafo de código del repositorio en el mecanismo de atención de un LLM y lo combina con un marco RAG de grafos sin agentes, alcanzando un 43 % en SWE‑bench Lite como el mejor modelo de peso abierto. **“GraphCodeAgent: Dual Graph‑Guided LLM Agent for Retrieval‑Augmented Repo‑Level Code Generation”** ([enlace](https://arxiv.org/abs/2504.10046)) de Li et al. usa un diseño de doble grafo (un grafo de requisitos y un grafo estructural‑semántico de código) y permite que el agente haga multi‑hop a través de ambos para la recuperación. **“Knowledge Graph Based Repository‑Level Code Generation”** ([enlace](https://arxiv.org/abs/2505.14394)) de Athale y Vaddina representa un repositorio como un grafo que captura información estructural y relacional y emplea recuperación híbrida sobre él.

La tercera literatura es la de operaciones, y de ahí proviene la capa cuatro. La ingeniería de confiabilidad del sitio (SRE) lleva runbooks desde hace dos décadas: procedimientos breves y opinados vinculados a una alerta o incidente recurrente. El runbook indica al ingeniero de guardia qué panel abrir, qué servicio reiniciar, qué informe post‑mortem generar. Los agentes de codificación necesitan el mismo artefacto, pero vinculado a solicitudes de usuario recurrentes en lugar de páginas. Las habilidades **Claude Code** de Anthropic y los comandos de **Cursor** son versiones preliminares de esto. Son playbooks que un agente invoca, no grafos que un agente lee, pero la forma es la misma: intención, conceptos involucrados, archivos clave, pasos de validación. El precedente publicado más cercano a la idea de capa cuatro, tratando las tareas como nodos de primera clase en el mismo grafo de conocimiento que el sistema que tocan, es el artículo de 2024 **[Knowledge Graph Modeling‑Driven Large Language Model Operating System for Task Automation](https://arxiv.org/abs/2408.14494)**. Modela flujos de trabajo de ingeniería de procesos como nodos KG que el LLM recorre para ensamblar planes ejecutables. La capa cuatro es lo que ocurre cuando haces eso para una base de código: conviertes los playbooks en nodos de primera clase en el mismo grafo que la arquitectura que navegan, de modo que el agente pueda pasar de “qué hacer” a “qué significa” a “dónde vive” sin salir de la estructura.

Ninguno de esos trabajos nombra la división de cuatro capas como una unidad. Las capas uno y dos están bien documentadas. La capa tres se documenta como ontología y resumen de comunidad, pero la forma específica de “predicados humanos sobre comunidades de código, regeneradas como una caché de la capa dos” es una adaptación práctica más que un método publicado. La capa cuatro es la menos estandarizada, pero se alinea con ideas de flujo de trabajo de agentes y memoria procedimental en la literatura de operaciones. La contribución aquí es el empaquetado, no cualquiera de las capas individuales: grafo de código bruto → grafo de comunidad → ontología semántica → playbooks operacionales, con evidencia preservada en cada salto y dos direcciones de lectura válidas sobre la pila. *La Capa de Memoria* describe los “tres cerebros” de Graphify (Tree‑sitter para código, un extractor semántico para prosa, una canalización multimodal para diagramas y audio), pero esos son modos de extracción, no capas de navegación, y el libro se detiene en “construye el grafo y deja que el agente lo recorra”. GraphRAG genera resúmenes de comunidad pero los usa como anclajes de recuperación para evidencia a nivel de fragmento, no como un mapa legible permanente. Los artículos centrados en código exponen nodos y aristas crudas al LLM. O el modelo lee una masa estructural, o lee resúmenes en lenguaje natural colapsados para recuperación, o lee un archivo de habilidades desvinculado de cualquier contexto estructural. La división que me funcionó fue mantener la prosa humana como mapa de entrada, enlazarla de vuelta a los bordes estructurales agregados y los símbolos subyacentes, y colocar una delgada capa operativa encima para que las tareas recurrentes tengan una forma estable.
## Qué cuesta esto
