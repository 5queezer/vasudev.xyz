...
Ich habe letzte Woche einen Graphify‑Graphen für den Code eines Coding‑Agenten erstellt. Die erste Version war das Offensichtliche. Knoten waren Dateien, Klassen, Funktionen, Methoden. Kanten waren `imports`, `calls`, `contains`, `method`. Das Öffnen in Obsidian zeigte mir ein Haarball‑Diagramm. Ich clusterte es in Gemeinschaften und benannte jede mit einem Konzeptnamen. Besser. Aber die Kanten lauteten immer noch so etwas wie `calls×32 / method×7 / contains×11`. Nützlich als Beweismaterial, nicht als Karte. Dann schrieb ich die Kanten als menschliche Beziehungsphrasen um: „steuert und beobachtet Unterhaltungen“, „liefert Anmeldedaten an“, „rendert Markdown mit“. Plötzlich las sich der Graph wie ein Satz. Plötzlich konnte ein LLM sich daran orientieren.

Das ergab für mich drei Ebenen. Dann versuchte ich, sie bei einer realen Aufgabe („füge diesem CLI einen neuen Modell‑Provider hinzu“) zu nutzen und merkte, dass ich eine vierte Ebene brauchte.
...
Die erste ist Programmanalyse. Der [Code Property Graph](https://en.wikipedia.org/wiki/Code_property_graph), eingeführt von Yamaguchi et al. in ihrem IEEE S&P‑Paper 2014 „Modeling and Discovering Vulnerabilities with Code Property Graphs“ (das 2024 den IEEE Test‑of‑Time Award gewann), fasst bereits drei klassische Darstellungen zu einer Struktur zusammen: den abstrakten Syntaxbaum, den Kontrollflussgraphen und den Programmabhängigkeitsgraphen. Der ursprüngliche Anwendungsfall war die Entdeckung von Schwachstellen, aber die Lehre verallgemeinert. Eine einzelne Darstellung kann nicht alle Fragen beantworten, die Sie zu Code haben, also komponieren Sie Darstellungen und lassen die Abfrage den richtigen Ausschnitt wählen. Das ist Schicht eins gut umgesetzt, und das wird seit über einem Jahrzehnt gut gemacht.

Die zweite ist graphbasierte Retrieval‑Methodik für LLMs. Microsofts GraphRAG‑Paper, ["From Local to Global: A Graph RAG Approach to Query‑Focused Summarization"](https://arxiv.org/abs/2404.16130) von Edge et al. (2024), betont explizit den Wert einer Zwischen‑Community‑Schicht. Sie bauen einen Entitäts‑Graphen, partitionieren ihn mit dem [Leiden‑Algorithmus](https://de.wikipedia.org/wiki/Leiden-Algorithmus) und erzeugen Zusammenfassungen pro Community. Die [GraphRAG‑Dokumentation](https://microsoft.github.io/graphrag/) ist eindeutig darüber, was diese Communities Ihnen bringen: Jede Ebene der resultierenden Hierarchie „repräsentiert ein unterschiedliches Abstraktions‑ und Zusammenfassungsniveau.“ Das ist die geschichtete Sichtweise in ihren eigenen Worten, angewandt auf Dokumente anstelle von Code. Abfragen treffen zuerst die Community‑Zusammenfassungen und steigen nur bei Bedarf zu Entitäten hinab. *The Memory Layer* beschreibt dasselbe Muster in Kapitel 5 und behandelt HybridRAG (eine einstellbare Mischung `α · vector_score + (1 - α) · graph_score`) als neuen Standard. Beide bestätigen, dass die Community‑Schicht real und tragfähig ist.

Neuere, code‑spezifische Arbeiten konvergieren auf dieselbe Form. ["Code Graph Model (CGM): A Graph‑Integrated Large Language Model for Repository‑Level Software Engineering Tasks"](https://arxiv.org/abs/2505.16901) von Tao et al. integriert die Repository‑Code‑Graph‑Struktur in den Aufmerksamkeitsmechanismus eines LLMs und kombiniert sie mit einem agentenlosen Graph‑RAG‑Framework, wobei es 43 % auf SWE‑bench Lite als bestes Open‑Weight‑Modell erreicht. ["GraphCodeAgent: Dual Graph‑Guided LLM Agent for Retrieval‑Augmented Repo‑Level Code Generation"](https://arxiv.org/abs/2504.10046) von Li et al. verwendet ein Dual‑Graph‑Design (ein Anforderungs‑Graph und ein struktur‑semantischer Code‑Graph) und lässt den Agenten über beide hinweg mehrstufig suchen. ["Knowledge Graph Based Repository‑Level Code Generation"](https://arxiv.org/abs/2505.14394) von Athale und Vaddina stellt ein Repository als Graph dar, der strukturelle und relationale Informationen erfasst, und nutzt hybride Retrieval‑Methoden darüber.

Die dritte Literatur ist die der Betriebsabläufe, und dorther stammt Schicht vier. Site‑Reliability‑Engineering nutzt seit zwei Jahrzehnten Runbooks: kurze, meinungsstarke Verfahren, die einem Alarm oder einem wiederkehrenden Incident zugeordnet sind. Das Runbook sagt dem On‑Call‑Ingenieur, welches Dashboard zu öffnen ist, welchen Service neu zu starten ist, welches Post‑Mortem zu erstellen ist. Coding‑Agents benötigen dasselbe Artefakt, jedoch zu wiederkehrenden Benutzeranfragen statt zu Seiten. Anthropics Claude‑Code‑Skills und Cursors Befehle sind frühe Versionen davon. Sie sind Playbooks, die ein Agent aufruft, keine Graphen, die ein Agent liest, aber die Form ist dieselbe: Intent, beteiligte Konzepte, Schlüsseldateien, Validierungsschritte. Das engste veröffentlichte Vorbild für die Schicht‑vier‑Idee, bei der Aufgaben selbst als erstklassige Knoten im selben Wissensgraphen wie das System, das sie berühren, behandelt werden, ist das 2024‑Paper [Knowledge Graph Modeling‑Driven Large Language Model Operating System for Task Automation](https://arxiv.org/abs/2408.14494). Es modelliert Prozess‑Engineering‑Workflows als KG‑Knoten, die das LLM traversiert, um ausführbare Pläne zu erstellen. Schicht vier ist das, was passiert, wenn Sie das für einen Code‑Base tun: Playbooks zu erstklassigen Knoten im selben Graphen wie die Architektur, die sie navigieren, sodass der Agent von „was zu tun ist“ über „was das bedeutet“ zu „wo es lebt“ wandern kann, ohne die Struktur zu verlassen.

Keines dieser Werke benennt die Vier‑Schichten‑Aufteilung als Einheit. Schicht eins und zwei sind stark dokumentiert. Schicht drei wird als Ontologie und Community‑Zusammenfassung dokumentiert, aber die konkrete Form von „menschlichen Prädikaten über Code‑Communities, regeneriert als Cache von Schicht zwei“ ist eine praktische Anpassung statt einer veröffentlichten Methode. Schicht vier ist am wenigsten standardisiert, passt aber zu Agent‑Workflow‑ und Procedural‑Memory‑Ideen in der Operations‑Literatur. Der Beitrag hier ist das Packaging, nicht irgendeine der einzelnen Schichten: roher Code‑Graph → Community‑Graph → semantische Ontologie → operative Playbooks, mit Belegen, die bei jedem Sprung erhalten bleiben, und zwei gültigen Lesestrichen über den Stack. *The Memory Layer* beschreibt Graphifys „drei Gehirne“ (Tree‑sitter für Code, einen semantischen Extraktor für Prosa, eine multimodale Pipeline für Diagramme und Audio), aber das sind Extraktions‑Modalitäten, keine Navigations‑Schichten, und das Buch endet bei „den Graph bauen und den Agenten ihn wandern lassen.“ GraphRAG erzeugt Community‑Zusammenfassungen, nutzt sie aber als Retrieval‑Anker für Chunk‑level‑Belege, nicht als permanente menschenlesbare Karte. Die code‑spezifischen Arbeiten setzen rohe Knoten und Kanten dem LLM aus. Entweder liest das Modell strukturelles „Matsch“, oder es liest natürlichsprachliche Zusammenfassungen, die für Retrieval komprimiert wurden, oder es liest eine Skill‑Datei, die von jeglichem strukturellen Kontext losgelöst ist. Die Aufteilung, die für mich funktionierte, besteht darin, die menschliche Prosa als Einstiegskarte zu behalten, sie zurück zu den aggregierten strukturellen Kanten und den Symbolen darunter zu verlinken und eine dünne operative Schicht obenauf zu legen, damit wiederkehrende Aufgaben eine stabile Form erhalten.
## Was das kostet
