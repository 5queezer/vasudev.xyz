...
agentQuestions:
  ...
  - "Was fügt Schicht vier für Agenten hinzu?"
  - "Wie sollte ein LLM einen Codegraphen navigieren?"
description: "Der Speicher sollte ein Graph sein. Der Graph sollte vier Schichten haben. Die mittlere Schicht übernimmt die Navigation. Die oberste Schicht wandelt Navigation in Aktionen um."
...
---
Andrej Karpathy brachte in einem Beitrag das Argument vor: Speicher ist eine Struktur, die man bewohnt, nicht ein Cache, den man bei jedem Prompt auffüllt. Zweiundvierzig Stunden später veröffentlichte Safi Shamsi [Graphify](https://github.com/safishamsi/graphify) und schrieb *The Memory Layer* dazu. Die These lautet, dass ein KI‑Agent einen Wissensgraphen durchwandern sollte, anstatt einen Vektor‑Index zu durchsuchen, weil Beziehungen im Weißraum zwischen den Chunks liegen und ein Vektorspeicher sie nicht erkennen kann.

Ich stimme der These zu. Ich finde, das Buch bleibt eine Ebene zu kurz. Dann habe ich das gepostet und sofort bemerkt, dass es sogar zwei Ebenen zu kurz ist.

Ich habe letzte Woche einen Graphify‑Graphen für den Code einer Coding‑Agent‑Codebase erzeugt. Die erste Version war das Offensichtliche. Knoten waren Dateien, Klassen, Funktionen, Methoden. Kanten waren `imports`, `calls`, `contains`, `method`. Das Öffnen in Obsidian ergab ein Haarball‑Diagramm. Ich clusterte es in Gemeinschaften und beschriftete jede mit einem Konzeptnamen. Besser. Aber die Kanten zeigten noch Dinge wie `calls×32 / method×7 / contains×11`. Nützlich als Beweis. Nicht nützlich als Karte. Dann schrieb ich die Kanten als menschliche Beziehungsphrasen um: „steuert und beobachtet Unterhaltungen“, „stellt Anmeldeinformationen bereit für“, „rendert Markdown mit“. Plötzlich las sich der Graph wie ein Satz. Plötzlich konnte ein LLM sich daran orientieren.

Das gab mir drei Ebenen. Dann versuchte ich, sie bei einer echten Aufgabe („einen neuen Model‑Provider zu diesem CLI hinzufügen“) zu verwenden und merkte, dass ich eine vierte Ebene brauchte.
...
Die erste ist Programmanalyse. Das [Code Property Graph](https://de.wikipedia.org/wiki/Code_property_graph), eingeführt von Yamaguchi et al. in ihrem 2014 IEEE S&P‑Paper „Modeling and Discovering Vulnerabilities with Code Property Graphs“ (das 2024 den IEEE Test‑of‑Time Award gewann), fasst bereits drei klassische Darstellungen zu einer Struktur zusammen: den abstrakten Syntaxbaum, den Kontrollflussgraphen und den Program Dependence Graph. Der ursprüngliche Anwendungsfall war die Schwachstellenentdeckung, aber die Lehre verallgemeinert. Eine einzelne Darstellung kann nicht alle Fragen beantworten, die Sie zu Code haben, also komponieren Sie Darstellungen und lassen die Abfrage den richtigen Ausschnitt wählen. Das ist Ebene 1 gut umgesetzt, und das wird seit über einem Jahrzehnt gut gemacht.

Die zweite ist graphbasierte Retrieval für LLMs. Microsofts GraphRAG‑Paper, „From Local to Global: A Graph RAG Approach to Query‑Focused Summarization“ (https://arxiv.org/abs/2404.16130) von Edge et al. (2024), ist explicit bezüglich des Wertes einer Zwischenschicht – der Community‑Ebene. Sie bauen einen Entitätsgraphen, partitionieren ihn mit dem [Leiden‑Algorithmus](https://de.wikipedia.org/wiki/Leiden_algorithm), und erzeugen Zusammenfassungen pro Community. Die [GraphRAG‑Dokumentation](https://microsoft.github.io/graphrag/) spricht klar darüber, was diese Communities Ihnen bringen: Jede Ebene der resultierenden Hierarchie „repräsentiert ein unterschiedliches Abstraktions‑ und Zusammenfassungs‑Level.“ Das ist das geschichtete Rahmenwerk in ihren eigenen Worten, angewandt auf Dokumente statt Code. Abfragen treffen zunächst die Community‑Zusammenfassungen und sinken nur bei Bedarf in Entitäten ab. *The Memory Layer* beschreibt dasselbe Muster in Kapitel 5 und behandelt HybridRAG (eine einstellbare Mischung `α·vector_score + (1‑α)·graph_score`) als den neuen Standard. Beide bestätigen, dass die Community‑Ebene real und tragfähig ist.

Neuere, code‑spezifische Arbeiten konvergieren auf dieselbe Form. „[Code Graph Model (CGM): A Graph‑Integrated Large Language Model for Repository‑Level Software Engineering Tasks](https://arxiv.org/abs/2505.16901)“ von Tao et al. integriert die Repository‑Code‑Graph‑Struktur in den Aufmerksamkeits‑Mechanismus eines LLM und verbindet sie mit einem agentenlosen Graph‑RAG‑Framework, wobei es 43 % auf SWE‑bench Lite als bestes Open‑Weight‑Modell erreicht. „[GraphCodeAgent: Dual Graph‑Guided LLM Agent for Retrieval‑Augmented Repo‑Level Code Generation](https://arxiv.org/abs/2504.10046)“ von Li et al. verwendet ein Dual‑Graph‑Design (ein Requirements‑Graph und ein strukturell‑semantischer Code‑Graph) und lässt den Agenten über beide für Retrieval mehrstufig hopsen. „[Knowledge Graph Based Repository‑Level Code Generation](https://arxiv.org/abs/2505.14394)“ von Athale und Vaddina repräsentiert ein Repo als Graph, der strukturelle und relationale Informationen erfasst und hybride Retrieval‑Methoden darüber nutzt.

Die dritte Literatur ist die Operations‑Literatur, und hierher stammt Ebene 4. Site Reliability Engineering hat seit zwei Jahrzehnten Runbooks: kurze, meinungsstarke Verfahren, die an einen Alarm oder ein wiederkehrendes Incident gekoppelt sind. Das Runbook sagt dem On‑Call‑Ingenieur, welches Dashboard er öffnen, welchen Service er neu starten und welches Post‑Mortem er ausfüllen soll. Coding‑Agents benötigen dasselbe Artefakt, jedoch an wiederkehrende Nutzeranfragen statt Seiten gebunden. Anthropics Claude Code‑Skills und Cursors Befehle sind Frühstadien‑Versionen dessen. Sie sind Playbooks, die ein Agent aufruft, nicht Graphen, die ein Agent liest, aber die Form ist dieselbe: Absicht, beteiligte Konzepte, Schlüsseldateien, Validierungsschritte. Der engste veröffentlichte Präzedenzfall für die Ebene‑vier‑Idee, bei der Aufgaben selbst als erstklassige Knoten im selben Knowledge‑Graph wie das System, das sie berühren, behandelt werden, ist das 2024‑Paper [Knowledge Graph Modeling‑Driven Large Language Model Operating System for Task Automation](https://arxiv.org/abs/2408.14494). Es modelliert Prozess‑Engineering‑Workflows als KG‑Knoten, die das LLM traversiert, um ausführbare Pläne zusammenzustellen. Ebene 4 ist das, was passiert, wenn man das für einen Code‑Base macht: Playbooks zu erstklassigen Knoten im selben Graph wie die Architektur, die sie navigieren, damit der Agent von „was zu tun ist“ über „was das bedeutet“ zu „wo es lebt“ wandern kann, ohne die Struktur zu verlassen.

Keines dieser Werke nennt die Vier‑Ebenen‑Aufteilung als Einheit. Ebenen 1 und 2 sind stark dokumentiert. Ebene 3 ist als Ontologie‑ und Community‑Zusammenfassung dokumentiert, aber die konkrete Form von „menschlichen Prädikaten über Code‑Communities, regeneriert als Cache von Ebene 2“ ist eine praktische Anpassung statt einer veröffentlichten Methode. Ebene 4 ist am wenigsten standardisiert, passt aber zu Agent‑Workflow‑ und Procedural‑Memory‑Ideen in der Operations‑Literatur. Der Beitrag hier ist das Packaging, nicht irgendeine der einzelnen Ebenen: roher Code‑Graph → Community‑Graph → semantische Ontologie → operative Playbooks, mit Beweisen, die bei jedem Hop erhalten bleiben, und zwei gültigen Les‑Richtungen über den Stack. *The Memory Layer* beschreibt Graphifys „drei Gehirne“ (Tree‑sitter für Code, ein semantischer Extraktor für Prosa, eine multimodale Pipeline für Diagramme und Audio), aber das sind Extraktions‑Modalitäten, keine Navigations‑Ebenen, und das Buch stoppt bei „baue den Graph und lass den Agenten ihn durchlaufen.“ GraphRAG erzeugt Community‑Zusammenfassungen, nutzt sie aber als Retrieval‑Anker für Chunk‑Level‑Beweise, nicht als permanente menschenlesbare Karte. Die code‑spezifischen Arbeiten setzen rohe Knoten und Kanten dem LLM aus. Entweder liest das Modell strukturellen Brei, oder es liest natural‑language‑Zusammenfassungen, die für Retrieval komprimiert wurden, oder es liest eine Skill‑Datei, die von jedem strukturellen Kontext losgelöst ist. Die Aufteilung, die für mich funktioniert hat, ist, den menschlichen Text als Einstiegskarte zu behalten, ihn zurück zu den aggregierten strukturellen Kanten und den darunterliegenden Symbolen zu verlinken und eine dünne operative Schicht darüberzulegen, sodass wiederkehrende Aufgaben eine stabile Form bekommen.
## Was das kostet
