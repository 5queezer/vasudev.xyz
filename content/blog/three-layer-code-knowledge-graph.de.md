---
title: "Ihr Code‑Wissensgraph benötigt vier Schichten, nicht nur eine"
date: 2026-05-01
...
description: "Gedächtnis sollte ein Graph sein. Der Graph sollte vier Schichten haben. Die mittlere Schicht trägt die Navigation. Die obere Schicht wandelt Navigation in Aktion um."
...
---
Andrej Karpathy machte das Argument in einem Beitrag: Speicher ist eine Struktur, die man bewohnt, kein Cache, den man bei jedem Prompt nachfüllt. Zweiundvierzig Stunden später veröffentlichte Safi Shamsi [Graphify](https://github.com/safishamsi/graphify) und schrieb *The Memory Layer* darum herum. Die These lautet, dass ein KI‑Agent einen Wissensgraphen durchlaufen sollte, anstatt einen Vektor‑Index zu durchsuchen, weil Beziehungen im Weißraum zwischen den Chunks liegen und ein Vektor‑Store sie nicht erkennen kann.
...
Ich habe letzte Woche einen Graphify‑Graphen für den Code‑Base eines Coding‑Agenten erzeugt. Die erste Version war das Offensichtliche. Knoten waren Dateien, Klassen, Funktionen, Methoden. Kanten waren `imports`, `calls`, `contains`, `method`. Als ich ihn in Obsidian öffnete, sah er aus wie ein Haarball. Ich clusterte ihn in Communities und benannte jede mit einem Konzeptnamen. Besser. Aber die Kanten zeigten immer noch Dinge wie `calls×32 / method×7 / contains×11`. Nützlich als Beweis, nicht als Karte. Dann schrieb ich die Kanten als menschliche Beziehungsphrasen um: „steuert und beobachtet Konversationen“, „stellt Anmeldeinformationen bereit für“, „rendert Markdown mit“. Plötzlich las sich der Graph wie ein Satz. Plötzlich konnte ein LLM sich daran orientieren.

Das gab mir drei Ebenen. Dann versuchte ich, sie bei einer echten Aufgabe („einen neuen Modell‑Provider zu diesem CLI hinzufügen“) zu verwenden und stellte fest, dass ich eine vierte brauchte.

**Ein Code‑Wissensgraph für einen LLM‑Agenten sollte vier Ebenen haben. Ebene 2 übernimmt die Navigation. Ebene 4 erledigt die Arbeit.**
...
Drei Literaturen konvergieren auf diese Form und sprechen meistens nicht miteinander.

Die erste ist die Programmanalyse. Das [Code Property Graph](https://de.wikipedia.org/wiki/Code_property_graph), eingeführt von Yamaguchi et al. in ihrem IEEE S&P‑Paper 2014 „Modeling and Discovering Vulnerabilities with Code Property Graphs“ (das 2024 den IEEE Test‑of‑Time Award gewann), fasst bereits drei klassische Darstellungen zu einer Struktur zusammen: den abstrakten Syntaxbaum, den Kontrollflussgraphen und den Programmadhängigkeitsgraphen. Der ursprüngliche Anwendungsfall war die Schwachstellenerkennung, aber die Lehre lässt sich verallgemeinern. Eine einzelne Darstellung kann nicht alle Fragen beantworten, die du zu Code hast, also komponierst du Darstellungen und lässt die Abfrage den richtigen Ausschnitt auswählen. Das ist Schicht 1 gut umgesetzt, und das wird seit über einem Jahrzehnt gut gemacht.

Die zweite ist die graphbasierte Retrieval‑Methode für LLMs. Microsofts GraphRAG‑Paper, ["From Local to Global: A Graph RAG Approach to Query‑Focused Summarization"](https://arxiv.org/abs/2404.16130) von Edge et al. (2024), betont explizit den Wert einer Zwischen‑Community‑Schicht. Sie bauen einen Entitätsgraphen, partitionieren ihn mit dem [Leiden‑Algorithmus](https://de.wikipedia.org/wiki/Leiden_algorithmus) und erzeugen Zusammenfassungen pro Community. Die [GraphRAG‑Dokumentation](https://microsoft.github.io/graphrag/) sagt klar, was diese Communities dir bringen: Jede Ebene der resultierenden Hierarchie „repräsentiert ein unterschiedliches Abstraktions‑ und Zusammenfassungsniveau.“ Das ist die geschichtete Darstellung in ihren eigenen Worten, angewandt auf Dokumente statt auf Code. Abfragen treffen zunächst die Community‑Zusammenfassungen und steigen nur bei Bedarf in die Entitäten ab. *The Memory Layer* beschreibt dasselbe Muster in Kapitel 5 und behandelt HybridRAG (eine abstimmbare Mischung `α · vector_score + (1 - α) · graph_score`) als den neuen Standard. Beide bestätigen, dass die Community‑Schicht real und belastungsfähig ist.

Aktuelle Code‑spezifische Arbeiten konvergieren auf dieselbe Form. ["Code Graph Model (CGM): A Graph‑Integrated Large Language Model for Repository‑Level Software Engineering Tasks"](https://arxiv.org/abs/2505.16901) von Tao et al. integriert die Repository‑Code‑Graph‑Struktur in den Attention‑Mechanismus eines LLM und kombiniert sie mit einem agentenlosen Graph‑RAG‑Framework, wodurch 43 % auf SWE‑bench Lite als bestes Open‑Weight‑Modell erreicht werden. ["GraphCodeAgent: Dual Graph‑Guided LLM Agent for Retrieval‑Augmented Repo‑Level Code Generation"](https://arxiv.org/abs/2504.10046) von Li et al. nutzt ein Dual‑Graph‑Design (ein Anforderungs‑Graph und ein strukturell‑semantischer Code‑Graph) und lässt den Agenten über beide für Retrieval mehrstufig hopsen. ["Knowledge Graph Based Repository‑Level Code Generation"](https://arxiv.org/abs/2505.14394) von Athale und Vaddina stellt ein Repo als Graph dar, der strukturelle und relationale Informationen erfasst, und verwendet hybride Retrieval‑Methoden darüber.

Die dritte Literatur ist die Operations‑Literatur, und hierher stammt Schicht 4. Site‑Reliability‑Engineering nutzt seit zwei Jahrzehnten Runbooks: kurze, einschlägige Verfahren, die an einen Alarm oder ein wiederkehrendes Incident gekoppelt sind. Das Runbook sagt dem on‑call Engineer, welches Dashboard zu öffnen, welchen Service neu zu starten und welches Post‑Mortem zu erstellen ist. Coding‑Agents benötigen dasselbe Artefakt, nur dass es an wiederkehrende Benutzeranfragen statt an Seiten gekoppelt ist. Anthropics Claude‑Code‑Skills und Cursors Befehle sind Frühstadien‑Versionen davon. Es sind Playbooks, die ein Agent aufruft, nicht Graphen, die ein Agent liest, aber die Form ist dieselbe: Intent, beteiligte Konzepte, Schlüssel‑Dateien, Validierungsschritte. Das veröffentlichteste Präzedenzbeispiel für die Schicht‑4‑Idee, bei dem Aufgaben selbst als First‑Class‑Knoten im selben Wissensgraphen wie das betroffene System behandelt werden, ist das 2024‑Paper [Knowledge Graph Modeling‑Driven Large Language Model Operating System for Task Automation](https://arxiv.org/abs/2408.14494). Es modelliert Prozess‑Engineering‑Workflows als KG‑Knoten, die das LLM traversiert, um ausführbare Pläne zu erstellen. Schicht 4 ist das, was passiert, wenn du das für einen Code‑Base machst: Playbooks werden zu First‑Class‑Knoten im selben Graph wie die Architektur, sodass der Agent von „was zu tun ist“ über „was es bedeutet“ zu „wo es lebt“ navigieren kann, ohne die Struktur zu verlassen.

Keines dieser Werke benennt die Vier‑Schichten‑Aufteilung als Einheit. Schicht 1 und 2 sind stark dokumentiert. Schicht 3 ist als Ontologie und Community‑Zusammenfassung dokumentiert, aber die konkrete Form von „human predicates over code communities, regenerated as a cache of layer two“ ist eher eine praktische Anpassung als eine veröffentlichte Methode. Schicht 4 ist am wenigsten standardisiert, passt aber zu Agent‑Workflow‑ und Procedural‑Memory‑Ideen in der Operations‑Literatur. Der Beitrag hier ist die Verpackung, nicht die einzelnen Schichten: roher Code‑Graph → Community‑Graph → semantische Ontologie → operative Playbooks, mit Beweisen, die bei jedem Hop gespeichert werden, und zwei gültigen Lesrichtungen über den Stack. *The Memory Layer* beschreibt Graphifys „drei Gehirne“ (Tree‑sitter für Code, einen semantischen Extraktor für Prosa, eine multimodale Pipeline für Diagramme und Audio), aber das sind Extraktionsmodalitäten, keine Navigationsschichten, und das Buch endet bei „den Graph bauen und den Agenten ihn wandern lassen.“ GraphRAG erzeugt Community‑Zusammenfassungen, nutzt sie aber als Retrieval‑Anker für Chunk‑Level‑Beweise, nicht als permanente menschenlesbare Karte. Die code‑spezifischen Arbeiten geben rohe Knoten und Kanten an das LLM. Entweder liest das Modell strukturellen Brei, oder es liest natürlichsprachige Zusammenfassungen, die für Retrieval komprimiert wurden, oder es liest eine Skill‑Datei, die von jedem strukturellen Kontext losgelöst ist. Die Aufteilung, die für mich funktionierte, ist, die menschliche Prosa als Einstiegskarte zu behalten, sie zurück zu den aggregierten strukturellen Kanten und Symbolen zu verlinken und obenauf eine dünne operative Schicht zu legen, sodass wiederkehrende Aufgaben eine stabile Form erhalten.
## Was das kostet
