...
Cada agente de IA hoy habla a través de una ventana de chat. El usuario hace una pregunta, el agente transmite tokens de vuelta, quizás con un bloque de código. Todo el frontend es un emulador de terminal glorificado.
...
| | Vercel AI SDK | TanStack AI | AG-UI | A2UI | Kombai |
|---|---|---|---|---|---|
| **Qué incluye** | Salida de herramienta (RSC pausado) | Salida de herramienta (render en cliente) | Eventos de interacción | Árboles UI JSON declarativos | Código fuente generado |
...
| **Modelo de seguridad** | Ejecución confiable | Código en sandbox (Node, Workers, QuickJS) | Validación de esquema de eventos | Catálogo de componentes en lista blanca | Salida estática, sin riesgo en tiempo de ejecución |
...
| **Cuándo elegir** | Cuando despliegas en Vercel y quieres el AI Gateway | Cuando buscas portabilidad y afinado por tipo de modelo | Cuando necesitas observabilidad del agente y humano en el bucle | Cuando necesitas multiplataforma desde agentes no confiables | Cuando conviertes diseños de Figma a código |

Eso es el tablero. Ahora las opiniones.
