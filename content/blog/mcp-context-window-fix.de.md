---
title: "Ihr MCP‑Setup verbraucht 90 % seines Kontextfensters. Hier ist die Lösung."
date: 2026-04-10
tags: ["mcp", "claude", "ai", "agents"]
agentQuestions:
  - "Warum verbrennt MCP das Kontextfenster?"
  - "Wie behebt das verzögerte Laden von Tools das MCP?"
  - "Was sollten MCP-Server im Voraus bereitstellen?"
description: "Jedes MCP-Tool, das du verbindest, lädt sein vollständiges Schema im Voraus, bevor du ein Wort tippst. Anthropic's deferred loading behebt das."
...
---
Connect [GitHub's MCP server](https://github.com/github/github-mcp-server) to Claude. Now check your token counter before sending a single message. [46.000 Tokens, 22 % des Claude‑Opus‑Kontextfensters](https://www.candede.com/articles/claude-tool-search), verbraucht von Werkzeugdefinitionen, die du noch nicht verwendet hast. Fügt man Jira (weitere ~17 K), einen Slack‑Server, Google Drive hinzu, kommt man schnell auf über 100 K Tokens Overhead, bevor irgendeine eigentliche Arbeit beginnt. [Anthropic hat interne Setups benchmarked, die allein bei Werkzeugdefinitionen 134 K Tokens erreichen](https://www.anthropic.com/engineering/advanced-tool-use).

**Jedes MCP‑Werkzeug, das du verbindest, ist eine im Voraus zu zahlende Steuer, egal ob das Werkzeug benutzt wird oder nicht.**

Dies ist das Standardverhalten von MCP‑Clients heute: Alle Werkzeugdefinitionen zu Beginn jeder Anfrage in den Kontext laden. Die Spezifikation verlangt das nicht. Es ist einfach der Weg des geringsten Widerstands, und er skaliert schlecht.
## Warum das passiert
...
**Eigene Suchimplementierungen.** Sie können Ihr eigenes Suchwerkzeug mit Embeddings oder semantischer Suche implementieren und `tool_reference`‑Blöcke zurückgeben. Das ist der richtige Weg für große Kataloge (1.000+ Werkzeuge), bei denen die BM25‑Abrufgenauigkeit nicht ausreicht. Anthropics [Code‑Execution‑Beitrag mit MCP](https://www.anthropic.com/engineering/code-execution-with-mcp) behandelt das allgemeinere Muster, MCP‑Server als Code‑APIs statt direkter Werkzeugaufrufe zu präsentieren. Lohnenswert zu lesen als Ergänzung.

**Agent‑SDK‑Unterstützung.** Anfang 2026 stellt das Python Agent SDK `defer_loading` nicht als Parameter zur Verfügung. Sie müssen auf die rohe API ausweichen. [Dieses GitHub‑Issue](https://github.com/anthropics/claude-agent-sdk-python/issues/525) verfolgt das.

**Andere Modell‑Anbieter.** `defer_loading` ist ein Claude‑API‑Feature, kein MCP‑Protokoll‑Feature. OpenAI, Gemini und andere haben noch kein Äquivalent. Wenn Sie provider‑agnostische Agenten bauen, benötigen Sie stattdessen eine clientseitige Routing‑Schicht.
