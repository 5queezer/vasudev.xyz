...
agentQuestions:
  - "¿Por qué fue esto un error de Traefik, no un error de Sablier?"
  - "¿Qué cambió en el inicio del plugin de Traefik 3.5.3?"
  - "¿Cómo puedo reproducir la falla de middleware inválido?"
series: ["Field Notes"]
...
---
[Sablier](https://github.com/sablierapp/sablier) te ofrece escalado a cero al estilo Cloud‑Run para contenedores Docker auto‑alojados. Las solicitudes llegan a un middleware de proxy inverso, el middleware despierta el contenedor objetivo bajo demanda, y el contenedor se apaga de nuevo después de un tiempo de inactividad. Pasé una tarde rastreando un error esporádico `invalid middleware` que los usuarios de Sablier han estado reportando durante meses. El error no estaba en Sablier. El trabajo produjo tres artefactos públicos: un [repositorio de reproducción determinista](https://github.com/5queezer/sablier-traefik-repro), un [issue upstream](https://github.com/traefik/traefik/issues/13005), y un [pull request de corrección](https://github.com/traefik/traefik/pull/13006). Aquí hay cuatro cosas que vale la pena saber antes de depurar tu próximo error `invalid middleware`.

**Un refactor enviado en una versión puntual de Traefik hizo que, silenciosamente, cada arranque de plugin dependiera de que `plugins.traefik.io` fuera accesible, y ningún propietario de middleware estaba en posición de notarlo.**
